...
**2026-09-25 — Site to first value: how do the best products get a skeptical owner from first visit to "it caught a lead for me", and earn inbox and DM permission?** Method: about 50 WebSearch queries (WebFetch blocked for developers.google.com and plaid.com, so every outside claim is graded B or C from snippets), plus the flow read in code at `b7b1f72`: landing, sign-in, onboarding, Settings connect panels, privacy and terms. Findings: 12 patterns and 4 ICP traps. FollowUp is ahead on identity-before-access, optional sources and honest consent. It is behind on the promise at the Connect button, asking Google for calendar with no context, not handling a partial yes, ending onboarding on a count instead of a real person, and Instagram in-app browsers blocking Google sign-in. Top 5 ranked. Nothing decided. Written up in `ux-patterns/2026-09-25-site-to-first-value-benchmark.md`.

**2026-09-26 — Landing page conversion: which strategies have evidence they raise conversion, and which is FollowUp's page missing?** Method: 16 WebSearch queries (WebFetch blocked for unbounce.com), confidence graded high/medium/low per claim, plus our own page measured from the live render: 1,286 words, about grade 4.7, 9.6 screens, "Start free" ×7, no analytics installed. Findings: 15 strategies with a scorecard. Strongest gaps: no measurement at all; the case (price, trust, "the catch") arrives after screen 2 while NN/g puts 74% of attention in the first two screens; no real proof yet (correctly, none may be faked); the hero shows an abstract diagram rather than the job. Kept: one repeated goal, plain words, one-click Google sign-in, no fake urgency. The private-beta gate (R-012) caps stranger conversion at zero by design, and is recorded as intentional. Four founder decisions listed. Written up in `landing-page/2026-09-26-conversion-strategies.md`.
