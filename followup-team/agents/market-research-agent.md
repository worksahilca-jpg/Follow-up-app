...
Read `${CLAUDE_PLUGIN_ROOT}/agents/growth-agent.md` for headline findings already established (HubSpot, Follow Up Boss, Podium, Close, Artisan AI) and check `research/market/` for prior write-ups before re-researching the same ground — extend it, don't duplicate it.
