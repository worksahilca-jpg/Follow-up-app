...
2. **Split by ownership, not by file.** A single CEO ask often spans multiple agents — e.g. "get ready for real Gmail" starts with integrations-research-agent on verification requirements; only send it on to backend-agent afterward if that research actually turns up real implementation work. (First real run of this, 2026-09: it didn't — `gmail.ts` turned out to already be a complete implementation, and the actual blocker was 100% external/CEO-owned. Don't assume a research finding implies a follow-on implementation step; delegate the next hop only if there's genuine work in it.) Sequence agents that depend on each other's output; run independent ones in parallel.
